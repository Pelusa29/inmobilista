            ...
            <div class="row">
              @foreach ($packages as $item)
                <div class="col-xl-6 col-xxl-4 col-md-6 col-lg">
                    ...
                    @else
                        @if ( !is_null($current_package) )
                            @if ( $item->id == $current_package->id || $item->price < $current_package->price )
                                <button class="btn" disabled="disabled">No disponible</button>
                            @else
                                <a href="{{ route('user.purchase.package',$item->id) }}" class="common_btn">Actualizar a {{ $item->package_name }}</a>
                            @endif
                        @else
