...
class PaymentController extends Controller
{
    ...
    {
        $promocode = PromotionalCode::where('promotional_code', $request->code)->first();

        return redirect()->route('user.purchase.package', [$promocode->package_id, $promocode->promotional_code]);
    }


    public function purchase($id, $code = null)
    {
        ...
        if (!is_null($current_package)) {
            if ($package->id == $current_package->id || $package->price < $current_package->price) {
                $notification = array('messege' => "Packete no disponible", 'alert-type' => 'error');

                return redirect()->back()->with($notification);
            }

            $price = $package->price - $current_package->price;
        }
        ...
        if ($package) {
            if ($package->package_type == 0) {
                ...
                $binance = true;
                // dd($package);
                //return view('user.profile.payment_new', compact('websiteLang','package','currency', 'price', 'code'));
                return view('user.profile.payment', compact('banner_image', 'menus', 'currency', 'setting', 'stripe', 'package', 'websiteLang', 'package_price', 'razorpay', 'paymentSetting', 'flutterwave', 'user', 'paystack', 'instamojo', 'zinli', 'binance'));
            }
        } else {
            $notify_lang = NotificationText::all();
    ...
    public function transfer(Request $request, Package $package)
    {
         ...
         * VALIDAR
         *******************************/
        if ($request->has('promotional_code') && $request->get('promotional_code') != "") {
            $promoCode = PromotionalCode::where('promotional_code', $request->promotional_code)
            ...
            if (!$promoCode) {
                $request->validate([
                    'transaction' => 'required|regex:/^[0-9]{12}$/',
                    'file'        => 'required|image',
                ], [
                    'transaction.required'  => 'Código promocional no es válido, es necesario añadir un número de transacción.',
                    'transaction.regex'     => 'Introduce un número de transacción de 12 dígitos.',
                    'file.required'         => 'Código promocional no es válido, es necesario subir un comprobante de pago.',
                    'file.image'            => 'Por favor, ingresa una imágen.',
                ]);
            }
        } else {
            $request->validate([
                ...
                'transaction.required'  => 'Es necesario añadir un número de transacción.',
                'transaction.regex'     => 'Introduce un número de transacción de 12 dígitos.',
                'file.required'         => 'Es necesario subir un comprobante de pago.',
                'file.image'            => 'Por favor, ingresa una imágen.',
            ]);
        }
        ...
        $wire_transaction = $request->transaction;
        if (!is_null($promoCode)) {
            ...
            $payment_status   = 1;
            $payment_method   = "Código de descuento";
            $wire_transaction = $promoCode->promotional_code;
            $file_path        = null;
        } else {
            $image          = $request->file;
            $file_extension = $image->getClientOriginalExtension();

            $file_name      = 'ticket-' . date('Y-m-d-h-i-s-') . rand(999, 9999) . '.' . $file_extension;
            $file_path      = 'uploads\tickets\\' . $file_name;

            $file = Image::make($image)->save(public_path() . '\\' . $file_path);
        }
        ...
        $order->transaction_id       = $wire_transaction;
        $order->payment_status       = $payment_status;
        $order->amount_usd           = $package->price;
    ...
    /* singli Payment method */
    public function zinliPayment(Request $request)
    {
        // project demo mode check
        if (env('PROJECT_MODE') == 0) {
        ...
        if ($request->has('promotional_code') && $request->get('promotional_code') != "") {
            ...
            if (!$promoCode) {
                $request->validate([
                    ...
                    'transaction.required'  => 'Código promocional no es válido, es necesario añadir un número de transacción.',
                    'transaction.regex'     => 'Introduce un número de transacción de 12 dígitos.',
                    'image.required'        => 'Código promocional no es válido, es necesario subir un comprobante de pago.',
                    'image.image'           => 'Por favor, ingresa una imágen.',
                ]);
            }
        } else {
            ...
            exit(); */
            $request->validate([
                'file_input' => 'required|mimes:png,jpg,jpeg|max:2048'
            ], [
                'file_input.required' => 'Es necesario subir un comprobante de pago.',
                'file_input.mimes'    => 'Por favor, ingresa una imágen con la siguiente extensión: png,jpg,jpeg.',
                'file_input.max'      => 'El tamaño máximo permitido es de 2MB.'
            ]);
        }
        ...
        $user = Auth::guard('web')->user();
        $amount_usd = round($package_info->price / $setting->currency_rate, 2);
        ...
        $order->user_id = $user->id;
        $order->order_id = '#' . rand(22, 44) . date('Ydmis');
        $order->package_id = $package_info->id;
        $order->purchase_date = date('Y-m-d');
        $order->expired_day = $package_info->number_of_days;
        $order->expired_date = $package_info->number_of_days == -1 ? null : date('Y-m-d', strtotime($package_info->number_of_days . ' days'));
        $order->payment_method = "Zinli Payment";
        $order->file = $file_path;
        $order->transaction_id = $request->tran_id;
        $order->payment_status = 0;
        $order->amount_usd = $amount_usd;
        $order->amount_real_currency = $package_info->price;
        $order->currency_type = $setting->currency_name;
        $order->currency_icon = $setting->currency_icon;
        $order->status = 0;
        $order->save();
        ...
        $message = str_replace('{{payment_method}}', 'Zinli Method', $message);
        $total_amount = $currency->currency_icon . $package_info->price;
    ...
    public function binancePayment(Request $request)
    {
        // project demo mode check
        if (env('PROJECT_MODE') == 0) {
            $notification = array('messege' => env('NOTIFY_TEXT'), 'alert-type' => 'error');
        ...
        $order->expired_day = $package_info->number_of_days;
        $order->expired_date = $package_info->number_of_days == -1 ? null : date('Y-m-d', strtotime($package_info->number_of_days . ' days'));
        $order->payment_method = "Binance Payment";
        $order->file = $file_path;
        $order->transaction_id = $request->tran_id;
        $order->payment_status = 0;
        $order->amount_usd = $amount_usd;
        ...
        $subject = $template->subject;
        $message = str_replace('{{user_name}}', $user->name, $message);
        $message = str_replace('{{payment_method}}', 'Binance Method', $message);
        $total_amount = $currency->currency_icon . $package_info->price;
        $message = str_replace('{{amount}}', $total_amount, $message);
        $message = str_replace('{{order_details}}', $order_details, $message);
        Mail::to($user->email)->send(new OrderConfirmation($message, $subject));
