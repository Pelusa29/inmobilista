...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $sections_about = AboutSection::all();
        $seo_text_about = SeoText::find(3);
        $menus = Navigation::all();

        $overviews = Overview::where('status', 1)->get();
        $properties_count = Property::where('status', 1)->count();
        $agents_count = User::where('status', 1)->count();
    ...
    public function aboutUs()
    {
        ...
        $agents = User::where('status', 1)->count();
        ...
        return view('user.about-us', compact('about', 'banner_image', 'overviews', 'properties', 'agents', 'partners', 'sections', 'seo_text', 'menus', 'websiteLang'));
    }
    ...
    public function agentkyc()
    {
        ...
        $banner_image = BannerImage::find(21);
        $paginate_qty = CustomPaginator::where('id', 3)->first()->qty;
        $agentskyc = User::where('statuskyc', 1)->orderBy('id', 'desc')->paginate($paginate_qty);
        $orders = Order::where(['statuskyc' => 1])->get();
        $default_profile_image = BannerImage::find(15);
        $seo_text = SeoText::find(5);
        $menus = Navigation::all();
        $websiteLang = ManageText::all();
    ...
    public function searchAgentPage(Request $request)
    {
        ...
        // Get agents (users) by properties
        $users = null;
        if ($request->has('city_id') and $request->get('city_id') != null) {
            $users = Property::where('city_id', $request->get('city_id'))->where('status', 1)->get()->pluck('user_id');
        } elseif ($request->has('state_id') and $request->get('state_id') != null) {
            $cities = City::where('country_state_id', $request->get('state_id'))->get()->pluck('id');
            $users = Property::whereIn('city_id', $cities)->where('status', 1)->get()->pluck('user_id');
        }

        if (is_null($users)) {
            $agents = User::where('status', 1)->orderBy('id', 'desc')->paginate($paginate_qty);
        } else {
            $agents = User::where('status', 1)->whereIn('id', $users)->orderBy('id', 'desc')->paginate($paginate_qty);
        }
